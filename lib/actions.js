...
exports.createAction = async (projectName) => {
    ...
    console.log(chalk.blueBright.dim(`Nanami CLI v${require('../package.json').version}`));
...
exports.removeRepoAction = (name) => {
    ...
    if(!customData.hasOwnProperty(name)){
        error(`"Can not found "${name}" in custom repository,please check with the command: ${chalk.gray('$') + " " + chalk.cyan('nanami ls')}`)
        return
    }
